...
(define-struct bird-state (y v-y))
(define-struct pipe (x gap-y))
(define-struct variables (gravity momentum scroll-speed))
(define-struct state-struct (bird-state pipes score variables abilities))
(define initial-variables (make-variables initial-gravity initial-momentum initial-scroll-speed))

(define (get-initial-state)
(make-state-struct
   (make-bird-state bird-initial-y 0)
   (cons (make-pipe scene-width (+ added-number (random random-threshold))) '())
   0 ; Scorul initial
   initial-variables
   (make-abilities null null) ; Nu avem abilitiati in initial-state
   ))
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-state y v-y) bird])
    (struct-copy bird-state bird [y (+ y v-y)] [v-y (+ v-y gravity)])))
...
(define (next-state-bird-onspace bird momentum)
    (struct-copy bird-state bird [v-y (- momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy state-struct current-state [bird-state (next-state-bird-onspace (get-bird current-state) (get-variables-momentum (get-variables current-state)))])
      current-state
      ))
...
(define (move-pipes pipes scroll-speed)
  (foldr
   (λ(pipe-tmp acc) (cons (struct-copy pipe pipe-tmp [x (- (get-pipe-x pipe-tmp) scroll-speed)]) acc))
   '()
   pipes))
...
(define (clean-pipes pipes)
  (filter (λ(pipe-tmp) (> (+ (get-pipe-x pipe-tmp) pipe-width) 0)) pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (reverse (cons (make-pipe 
                      (+ pipe-width pipe-gap (get-pipe-x (car (reverse pipes))))
...
(define (check-pipe-collisions bird pipes)
  (let loop ((pipe-list pipes) (collided? #f))
    (if (empty? pipe-list)
        collided?
        (loop (cdr pipe-list)
              (or
               ...
               (check-collision-rectangles
                ; pipe pos
                (match-let ([(pipe x gap-y) (car pipe-list)])
                  (make-posn x (- gap-y pipe-height)))
                (match-let ([(pipe x gap-y) (car pipe-list)])
                  (make-posn (+ x pipe-width) gap-y))
                ; bird pos
                (make-posn bird-x (get-bird-y bird))
                (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
                )
               ...
               (check-collision-rectangles
                ; pipe pos
                (match-let ([(pipe x gap-y) (car pipe-list)])
                  (make-posn x (+ gap-y pipe-self-gap)))
                (match-let ([(pipe x gap-y) (car pipe-list)])
                  (make-posn (+ x pipe-width) (+ gap-y pipe-self-gap pipe-height)))
                ; bird pos
                (make-posn bird-x (get-bird-y bird))
                (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))))))))
...
(define (next-state state)
  (match-let ([(state-struct bird-state pipes score variables abilities) state])
    (struct-copy state-struct state
                 [bird-state (next-state-bird bird-state (get-variables-gravity variables))]
                 [pipes (next-state-pipes pipes (get-variables-scroll-speed variables))]
                 [score (+ score 0.1)]
                 [variables (next-variables initial-variables abilities)]
                 [abilities (next-abilities abilities bird-state (get-variables-scroll-speed variables))])))
...
(define (draw-frame state)
  (match-let ([(state-struct bird-state pipes score variables abilities) state])
    (place-images
     ...
     (list (make-posn (+ bird-x (quotient bird-width 2)) (+ (get-bird-y (get-bird state)) (quotient bird-height 2)))
...
(define (place-pipes pipes scene)
  (if (empty? pipes)
      scene
      (match-let ([(pipe x gap-y) (car pipes)])
        (place-images
         (list pipe-image pipe-image)
         (list (make-posn (+ x (quotient pipe-width 2)) (- gap-y (quotient pipe-height 2)) )
               (make-posn (+ x (quotient pipe-width 2)) (+ gap-y (quotient pipe-height 2) pipe-self-gap)))
...
(define fast-ability (make-ability (hourglass "tomato") 30 null (λ(scroll-speed) (add1 scroll-speed))))
...
(define slow-ability (make-ability (hourglass "mediumseagreen") 10 null (λ(scroll-speed) (max 5 (sub1 scroll-speed)))))

; lista cu toate abilităţile posibile în joc
(define all-abilities-list (list fast-ability slow-ability))
...
(define (clean-abilities visible)
  (filter (λ(ability) (> (+ (posn-x (get-ability-pos ability)) (image-width (get-ability-image ability))) 0)) visible))
...
(define (move-abilities visible scroll-speed)
  (foldr
   (λ(vAbility acc) (cons (struct-copy ability vAbility [pos (make-posn (- (posn-x (get-ability-pos vAbility)) scroll-speed) (posn-y (get-ability-pos vAbility)))]) acc))
   '()
   visible))
...
(define (time-counter active)
  (foldr
   (λ(pActive acc) (if (<= (get-ability-time pActive) (/ 1 fps))
                       acc
                       (cons (struct-copy ability pActive [time (- (get-ability-time pActive) (/ 1 fps))]) acc)))
   '()
   active))
...
(define (next-abilities-visible visible scroll-speed)
  (if (empty? visible)
      (fill-abilities visible (- DISPLAYED_ABILITIES (length visible)) all-abilities-list)
      (clean-abilities (move-abilities (fill-abilities visible (- DISPLAYED_ABILITIES (length visible)) all-abilities-list) scroll-speed))))
...
(define (next-abilities pAbilities bird scroll-speed)
(struct-copy abilities pAbilities
               [visible (next-abilities-visible
                         (filter-not
                          (λ(pAbility) (check-collision-rectangles
                                        (make-posn bird-x (get-bird-y bird)) ; stanga sus
                                        (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height)) ; dreapta jos
                                        (match-let ([(posn ax ay) (get-ability-pos pAbility)]) ; stanga sus
                                          (make-posn (- ax (/ (image-width (get-ability-image pAbility)) 2)) (- ay (/ (image-height (get-ability-image pAbility)) 2))))
                                        (match-let ([(posn ax ay) (get-ability-pos pAbility)]) ; dreapta jos
                                          (make-posn (+ ax (/ (image-width (get-ability-image pAbility)) 2)) (+ ay (/ (image-height (get-ability-image pAbility)) 2))))))
                          (get-abilities-visible pAbilities))
                         scroll-speed)]
               [active (time-counter
                        (reverse
                         (foldl
                          (λ(pAbility acc) (cons pAbility acc))
                          (reverse (get-abilities-active pAbilities))
                          (filter
                           (λ(pAbility) (check-collision-rectangles
                                         (make-posn bird-x (get-bird-y bird)) ; stanga sus
                                         (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height)) ; dreapta jos
...
(define (next-variables pVariables abilities)
  (match-let ([(variables gravity momentum scroll-speed) pVariables])
    (struct-copy variables pVariables
                 [scroll-speed (if (empty? (get-abilities-active abilities))
                                   scroll-speed
                                   ((compose-abilities (get-abilities-active abilities)) scroll-speed))])))


; Folosind `place-image/place-images` va poziționa abilităţile vizibile la ability pos.
(define (place-visible-abilities visible scene)
      (let loop ((rVisible (reverse visible)) (current-scene scene)) 
        (if (empty? rVisible)
            current-scene
            (match-let ([(posn x y) (get-ability-pos (car rVisible))])
              (loop
               (cdr rVisible)
               (place-image (get-ability-image (car rVisible))
                            x 
                            y
                            current-scene))))))
...
(define (place-active-abilities active scene)
  (if (empty? active)
      scene
      (let loop ((current-actives active) (current-scene scene) (i 0))
        (if (empty? current-actives)
            scene
            (match-let ([(posn x y) abilities-posn])
              (place-image (scale 0.75 (get-ability-image (car current-actives)))
                           (- x (* 50 i))
                           y
                           (loop (cdr current-actives) current-scene (add1 i))))))))
